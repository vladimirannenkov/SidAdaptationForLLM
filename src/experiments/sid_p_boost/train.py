"""SID-P-Boost: тот же backbone-режим, что SID-P (sid_p_blocks/train.py —
первые k слоёв из E2E-чекпоинта, по умолчанию НЕ заморожен, обучается своим
multi-token лоссом), но верхние блоки обучаются не consistency-KL-формулой,
а BOOSTING-лоссом (src/sid/chunked.py::chunked_boost_loss) — адаптация
BoostResNet/gradient boosting в logit-space. Подробное объяснение идеи и
почему она даёт больше, чем consistency-KL: docs/boosting_explained.md.

Формула для блока i: L_i = sg(sum_{j<i} alpha_j*readout(h_j)) +
alpha_i*readout(h_i); loss_i = CE(target, softmax(L_i)) — блок учится
добавлять взвешенную коррекцию к уже накопленному (под stop-gradient)
предсказанию всех предыдущих глубин. alpha_i (src/sid/losses.py::BoostWeights)
— обучаемый скаляр на каждый верхний блок, инициализирован около нуля
(AdaLN-Zero принцип) — блок не может испортить ансамбль, пока не докажет
пользу собственным градиентом.

Две доп. абляции:
--freeze-readout except-last: общий readout обновляется только backbone'ом
  (если не заморожен) и последним верхним блоком.
--freeze-backbone-from <sid-чекпоинт>: backbone И readout загружаются из уже
  обученного (boosting) чекпоинта и полностью заморожены — верхние блоки
  переинициализируются с нуля и учатся поверх неподвижной, уже осмысленной
  пары backbone+readout.

Запуск (из корня репозитория):
    python src/experiments/sid_p_boost/train.py --k 6
    python src/experiments/sid_p_boost/train.py --smoke-test
"""
...
sys.path.insert(0, str(Path(__file__).resolve().parents[3]))  # src/experiments/sid_p_boost/ -> repo root
...
EXPERIMENT_NAME = "sid_p_boost"
EXPERIMENT_DIR = Path(__file__).resolve().parent


class HeadsAndWeights(nn.Module):
    """Обёртка только для чекпоинтинга: save_sid_checkpoint ожидает один
    модуль в слоте "heads" — здесь в нём и MultiTokenHeads backbone (может
    быть None, если backbone заморожен), и обучаемые alpha верхних блоков."""
...
def main():
    ...
    FREEZE_BACKBONE = args.freeze_backbone_from is not None
    ...
    if FREEZE_BACKBONE:
        ...
        print(f"backbone+readout загружены из {args.freeze_backbone_from} и ПОЛНОСТЬЮ ЗАМОРОЖЕНЫ "
              f"(источник: шаг {sid_ckpt['optimizer_step']}, {sid_ckpt['tokens_processed']:,} токенов)")
    else:
        ...
        print(f"backbone инициализирован из {args.source_checkpoint} "
              f"(шаг {source['optimizer_step']}, {source['tokens_processed']:,} токенов), НЕ заморожен")
    ...
    family = "SID-P-Boost"
    tags = [family, "wikitext103", f"vocab{config.vocab_size}", "boosting", f"k{K}", f"blocksize{UPPER_BLOCK_SIZE}"]
    ...
    experiment = init_experiment(
        ...
        parameters={
            ...
            "readout_frozen_mode": FREEZE_READOUT_MODE, "upper_block_loss": "boosting (cumulative logits)",
    ...
    print(f"k={K}, {num_upper_blocks} верхних блоков (block-size={UPPER_BLOCK_SIZE}, boosting); "
          f"freeze_readout={FREEZE_READOUT_MODE}, freeze_backbone={FREEZE_BACKBONE}; "
    ...
    def run_microbatch(accumulate, collect_metrics):
        ...
        for i, (start, end) in enumerate(block_layer_ranges):
            ...
            with ctx:
                ...
                ce_i = chunked_boost_loss(model, hidden_history, alpha_history, h_i, alpha_i, y,
                                           own_readout_fn=own_readout_fn)
    ...
    def estimate_val_loss():
        ...
        block_drift_sums = [0.0] * num_blocks_total
        ...
        return backbone_per_offset, [s / n for s in block_ce_sums], [s / n for s in block_drift_sums], \
            [s / n for s in block_cka_sums]
    ...
    for step in range(1, total_optimizer_steps + 1):
        ...
        if collect:
            ...
            block_str = " ".join(
                f"b{block_label(s, e)}(ce={c:.2f},drift={d:.3f},alpha={a:.3f},cka={k:.3f})"
                for (s, e), (c, d, a, k) in zip(block_layer_ranges, last_block_metrics))
        ...
        if step % validation_every == 0 or is_quarter:
            val_backbone_offset, val_block_ce, val_block_drift, val_block_cka = estimate_val_loss()
            val_metrics = {}
            ...
            for i, (start, end) in enumerate(block_layer_ranges):
                ...
                val_metrics[f"val/block{label}_drift"] = val_block_drift[i]
                val_metrics[f"val/block{label}_cka"] = val_block_cka[i]
            experiment.log_metrics(val_metrics, step=step)
            depth_str = " ".join(
                f"b{block_label(s, e)}:ppl={math.exp(c):.1f},cka={k:.3f}"
                for (s, e), c, k in zip(block_layer_ranges, val_block_ce, val_block_cka))
            ...
            print(f"step {step}: {backbone_str}глубина (ансамбль): {depth_str}")
            ...
            sid_config = {"family": family, "k": K, "upper_block_size": UPPER_BLOCK_SIZE,
                          ...
                          "source_checkpoint": None if FREEZE_BACKBONE else args.source_checkpoint,
                          "freeze_backbone_from": args.freeze_backbone_from, "backbone_frozen": FREEZE_BACKBONE,
                          "readout_frozen_mode": FREEZE_READOUT_MODE, "upper_block_loss": "boosting"}
    ...
    experiment.end()
    print(f"SID-P-Boost (k={K}, bs={UPPER_BLOCK_SIZE}, tag={TAG}) training loop завершён")
