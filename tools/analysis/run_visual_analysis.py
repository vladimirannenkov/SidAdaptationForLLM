"""Run the complete local Tuned Lens + CKA analysis for a SID-F checkpoint."""
...
def token_heatmap(trajectory, tokenizer, metric_name, path):
    ...
    axis.set_title(f"SID-F Tuned Lens {metric_name}: token trajectory")
...
def main():
    ...
    for metric_name, values in metrics.values.items():
        figure = heatmap(values.numpy(), f"SID-F Tuned Lens {metric_name}", "Token position", "Layer", artifact_dir / f"{metric_name}_heatmap.png", cmap="RdYlBu_r")
        ...
        axis.set_title(f"SID-F Tuned Lens {metric_name} by layer")
    ...
    experiment.add_tags(["TunedLens", "CKA", "SID-F", f"k{args.k}", "wikitext103"])
    experiment.log_parameters({"source_checkpoint": str(checkpoint_path), "k": args.k, "fit_steps": args.fit_steps, "eval_batches": args.eval_batches, "device": device, "checkpoint_optimizer_step": checkpoint_step, "checkpoint_tokens_processed": checkpoint_tokens})
    ...
    for name, figure in figures:
        experiment.log_figure(figure_name=name, figure=figure, step=0, format="png", metadata={"source_checkpoint": str(checkpoint_path), "layers": model.config.n_layer, "k": args.k})
        plt.close(figure)
