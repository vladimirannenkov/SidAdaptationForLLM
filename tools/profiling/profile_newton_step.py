"""Диагностика производительности chunked_boost_newton_loss — почему pilot
2026-09-22 оказался в разы медленнее ожидаемого даже после снятия форс-.item()
(sid/chunked.py). Замеряет wall-clock отдельно на backbone forward и на
каждый верхний блок (forward_range + chunked_boost_newton_loss + backward),
с explicit torch.cuda.synchronize() на границах — чтобы увидеть, где
реально уходит время, а не гадать по агрегированному tok/s.

Запуск: python -u tools/profile_newton_step.py (без -u вывод будет буферизован
до конца процесса, см. memory.md про stdout buffering)."""
...
N_MICROBATCHES = 3
print(f"MICRO_BATCH_SIZE={MICRO_BATCH_SIZE} CHUNK_SIZE={CHUNK_SIZE}", flush=True)
...
for mb in range(N_MICROBATCHES):
    ...
    print(f"[mb {mb}] backbone forward: {t_backbone * 1000:.1f} ms", flush=True)
    ...
    for i, (start, end) in enumerate(BLOCK_LAYER_RANGES):
        ...
        with ctx:
            loss_i, diag_i = chunked_boost_newton_loss(
                model, hidden_history, alpha_history, h_i, alpha_i, y, lam=0.01, beta=1.0,
        ...
        print(f"[mb {mb}] block{start}: fwd={t_fwd * 1000:.1f}ms loss={t_loss * 1000:.1f}ms "
              f"bwd={t_bwd * 1000:.1f}ms hist_len={len(hidden_history)} "
              f"alloc={mem_alloc:.0f}MiB reserved={mem_reserved:.0f}MiB", flush=True)
